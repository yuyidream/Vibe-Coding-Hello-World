...
## 2026年1月21日 - v3.1 BUG修复

### 🐛 BUG修复
- ✅ **修复访问日志无法记录首页访问的问题**
  - 问题：Nginx配置中API代理端口错误（指向Flask的5000端口而非FastAPI的8000端口）
  - 影响：用户访问首页时，访问日志无法记录到数据库
  - 修复：更新`nginx-backend.conf`，将API代理从5000端口改为8000端口

### 🔧 配置更新
- 📝 更新 `nginx-backend.conf`
  - 第24行：`proxy_pass http://127.0.0.1:5000` → `http://127.0.0.1:8000`
  - 第22行注释：更新为"API接口反向代理到FastAPI后端"
  - HTTPS配置注释中也同步更新

### 📦 新增文件
- `fix-access-log.sh` - 自动修复脚本
  - 自动检测Nginx配置位置（宝塔/系统Nginx）
  - 自动备份当前配置
  - 自动更新端口配置
  - 验证FastAPI服务状态
  - 测试并重新加载Nginx
- `BUG修复-访问日志.md` - 详细的问题分析和修复说明
  - 问题原因分析
  - 两种修复方案（自动/手动）
  - 验证步骤
  - 后续建议
...
### 📋 技术细节
- **根本原因**：项目从Flask迁移到FastAPI后，Nginx配置未同步更新
- **影响范围**：访问日志记录功能
- **严重程度**：中等（不影响网站正常访问）
- **修复状态**：✅ 已修复
...
```bash
# 测试API连通性
curl -X POST http://127.0.0.1/api/log
